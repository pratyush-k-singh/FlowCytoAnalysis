...
def calculate_statistics(data, columns):
    """
    Calculate mean, median, skew, variance, kurtosis, entropy, and standard deviation for each column.
    ...
    for column in columns:
        ...
        stats['kurtosis'].append(kurtosis(data[column]))
        
        # Calculate entropy, adding a small constant to avoid log(0) issues
        counts, bin_edges = np.histogram(data[column].dropna(), bins=10, density=True)
        stats['entropy'].append(entropy(counts + 1e-8))  # Adding a small constant to avoid zero counts
        
        # Calculate standard deviation
        stats['std_dev'].append(data[column].std())
    
    return stats

def normalize_statistics_logarithmic(statistics):
    """
    Normalize statistics to a logarithmic scale of 0 to 100.
    ...
    for key in statistics[0].keys():
        ...
        max_val = np.nanmax(log_values)
        normalized_values = 100 * log_values / max_val
...
def plot_heatmap(data, metric, columns, file_names, pdf, output_folder_path):
    ...
    pdf.savefig()
    plt.savefig(os.path.join(output_folder_path, f'{metric}_heatmap_logarithmic.png'), bbox_inches='tight')
    plt.close()
...
def main(input_folder_path, output_folder_path):
    ...
    normalized_stats = normalize_statistics_logarithmic(statistics)
    
    # Create the PDF file
    pdf_path = os.path.join(output_folder_path, 'Heatmaps_Logarithmic.pdf')
...
if __name__ == "__main__":
    ...
    output_folder_path = r'C:\Users\praty\cytoflow\Codeset\LogarithmicHeatmaps'
