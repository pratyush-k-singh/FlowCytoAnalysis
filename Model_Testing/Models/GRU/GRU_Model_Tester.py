...
def main():
    """
    Main function to load data, preprocess, predict using the GRU model, and merge with field data.
    """
    ...
    predictions = pd.DataFrame(y_pred, columns=[f'gru_{event}' for event in event_types])
    ...
    # Prepare the final DataFrame with GRU, XGB, and DNN predictions
    final_columns = ['id'] + [f'gru_{event}' for event in event_types] + \
