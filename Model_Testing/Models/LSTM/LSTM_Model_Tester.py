...
model_save_path = os.path.join(script_dir, 'LSTM_model.h5')
...
def load_ids_from_split_folder(split_folder):
    ...
    Parameters:
        split_folder (str): The path to the directory containing the split data.

    Returns:
        list of str: List of test IDs.
    """
...
def main():
    """
    Main function to load data, preprocess, predict using the LSTM model, and merge with field data.
    """
    ...
    predictions = pd.DataFrame(y_pred, columns=[f'lstm_{event}' for event in event_types])
    ...
    # Prepare the final DataFrame with LSTM, XGB, and DNN predictions
    final_columns = ['id'] + [f'lstm_{event}' for event in event_types] + \
